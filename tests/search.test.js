...
// Test both global and language-specific search pages
const searchPages = [
  { url: 'http://localhost:4321/search/', name: 'Global Search', hasLanguageFilters: true },
  { url: 'http://localhost:4321/en/search/', name: 'English Search', hasLanguageFilters: false },
  { url: 'http://localhost:4321/nl/search/', name: 'Dutch Search', hasLanguageFilters: false },
  { url: 'http://localhost:4321/de/search/', name: 'German Search', hasLanguageFilters: false },
  { url: 'http://localhost:4321/es/search/', name: 'Spanish Search', hasLanguageFilters: false }
];
...
describe('Search Page Functionality', () => {
  ...
  searchPages.forEach(({ url, name, hasLanguageFilters }) => {
    describe(`${name} (${url})`, () => {
      beforeEach(async () => {
      ...
      test('language filters should be functional', async () => {
        // Only test language filters on global search page
        if (!hasLanguageFilters) {
          return; // Skip for language-specific pages
        }
      ...
      test('clear filters button should reset all filters', async () => {
        ...
        // Set language filter only on global search page
        if (hasLanguageFilters) {
          await page.click('#lang-en');
        }
        ...
        // Only check language filters on global search page
        if (hasLanguageFilters) {
      ...
      test('search with URL parameter should work', async () => {
        ...
        await page.goto(url + '?q=experimentation', { waitUntil: 'networkidle2' });
