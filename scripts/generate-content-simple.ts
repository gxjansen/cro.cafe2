...
/**
 * Simple NocoDB Content Generation Script for GitHub Actions
 * Uses direct HTTP API calls instead of MCP
 */

import { SimpleContentGenerator } from '../src/lib/engines/simple-content-generator.js'
import { NocoDBAPIClient } from '../src/lib/services/nocodb-api-client.js'
...
async function main() {
  ...
  console.log('🚀 CROCAFE Simple Content Generator (Direct API)')
  console.log('===============================================')
  ...
  try {
    ...
    const client = new NocoDBAPIClient({
      baseUrl,
      apiKey,
      baseId
    })
...
function showHelp() {
  console.log(`
CROCAFE Simple Content Generator (Direct NocoDB API)

Usage: tsx scripts/generate-content-simple.ts [options]
...
Examples:
  tsx scripts/generate-content-simple.ts                    # Generate all content
  tsx scripts/generate-content-simple.ts --overwrite        # Overwrite existing files
  tsx scripts/generate-content-simple.ts -l en,nl           # Generate only EN and NL content
  tsx scripts/generate-content-simple.ts -d dist/content    # Custom output directory
...
Data Flow:
  NocoDB API → Simple Content Generator → MDX/JSON files → Astro Content Collections
`)
}
